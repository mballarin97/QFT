...
def left_compress(left, right):
    ...
    Parameters
    ----------
    left: list of N ndarrays
        left tensor
    right: list of N ndarrays
        right tensor
        
    Returns
    -------
    (left, right): tuple of tensors
        left is now a semi-unitary tensor
    """
...
def right_compress(left, right):
    ...
    -------
    (left, right): tuple of tensors
        right is now a semi-unitary tensor
    """
...
def right_canonize(sites):
    """
        ...
        Parameters
        ----------
        sites: list of N ndarrays
            Tensor to which we want to apply the right-canonization
        
        Returns
        -------
        sites: list of N ndarrays
            Right-canonized tensor
    """
...
def tensor_trace(MPS):
    ...
    Parameters
    ----------
    MPS: list of N ndarrays
        Matrix product state to contract
    
    Returns
    -------
    tensor_trace: double
        Contraction of MPS with itself
    """
...
def apply_two_qubit_gate_full(gate_matrix, pos, state):
    ...
    Parameters
    ----------
    gate_matrix: numpy array shape (4, 4)
        Matrix representation of the quantum gate
    pos: int
        Position of the first qubit where to apply *gate_matrix*. It will be applied to (*pos*, *pos*+1).
    state: list of N ndarrays
        Matrix product state to which we want to apply the gate
        
    Returns
    -------
    state: list of N ndarrays
        Matrix product state where we have applied the gate
    """
...
def apply_two_qubit_gate(gate_matrix, pos, state, chi=2):
    ...
    *chi* cannot be less than the maximum bond dimension of the initial mps *state*.
    ...
    state: list of N ndarrays
        Matrix product state to which we want to apply the gate
    chi: int, optional
        Bond dimension of the state after the qubit application. Default to 2
...
def apply_one_qubit_gate(gate, pos, state):
    ...
    Parameters
    ----------
    gate_matrix: numpy array shape (2, 2)
        Matrix representation of the quantum gate
    pos: int
        Position of the qubit where to apply *gate_matrix*. 
    state: list of N ndarrays
        Matrix product state to which we want to apply the gate
