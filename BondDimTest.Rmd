...
from MPS_QFT.circuit import GHZ_qiskit, reverse_qiskit, qiskit_get_statevect
```
...
## Fidelity w.r.t. gates with fixed bond dimension $\chi$

```{python}
from gates import cphase_swap_qiskit
# --- Parameters ---
tx = TestingData(N=5, d=2)
chi = 2
qc = QuantumCircuit(tx.N)
...
```{python}
MPS = quimb.tensor.tensor_1d.MatrixProductState.from_dense( np.matrix(random_state4MPS), 
                                                                         dims=[d for _ in range(tx.N)], max_bond=chi)

fid.append( Fidelity(MPS.to_dense()[:,0], qiskit_get_statevect(qc_ran)) )

for n in tqdm(range(tx.N, 0, -1)):
    if n == 0: pass
    elif n==1: 
        qc.h(0)
        qc_ran.h(0)
        ...
        qc_ran.h(0)
        for i in range(n-1):
            cphase_swap_qiskit(qc, i, i+1, np.pi*1/2**(i+1))
            cphase_swap_qiskit(qc_ran, i, i+1, np.pi*1/2**(i+1))
        
        MPS = quimb.tensor.tensor_1d.MatrixProductState.from_dense( np.matrix(random_state4MPS), 
                                                                         dims=[d for _ in range(tx.N)], max_bond=chi)
        MPS_1 = MPS_circ(qc, init_state=MPS)
        MPS_dense = MPS_1.to_dense()[:,0]
        res = qiskit_get_statevect(qc_ran)
        fid.append( Fidelity(res, MPS_dense) )
```
