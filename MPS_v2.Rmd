...
state = to_full_MPS(state, d=2, N=4)
```
...
rightmost = state[pos+2:]

d = left_tens.shape[0] #dimension of a qubit
chi = right_tens.shape[-1] #bound link between the group and the next (pos+2) qubit
print(f'd: {d}, chi: {chi}')
```
...
```{python}
gate_contracted.shape
```

```{python}
rho = right_contract(rightmost) #dim is chi x chi
```

```{python}
[s.shape for s in rightmost]
```
...
```{python}
gate_contracted_prime = ncon([gate_contracted,X],[[-1,-2,1],[1,-3]])

boundary_qubit = state[pos+2]
boundary_qubit_prime = ncon([Xinv, boundary_qubit], [[-1,1], [1,-2,-3]])

```

```{python}
gate_contracted_prime.shape
```

```{python}
#SVD
U, S, Vh = LA.svd(gate_contracted_prime.reshape(d, -1), full_matrices=False)
```

```{python}
left_new = U
right_new = np.diag(S) @ Vh
```

```{python}
contraction_dim = boundary_qubit_prime.shape[0]
```

```{python}
right_new = right_new.reshape(-1, d, contraction_dim)
```

```{python}
new_qubits = [left_new, right_new, boundary_qubit_prime, *rightmost[1:]]
```

```{python}
[s.shape for s in new_qubits]
```

```{python}
[bin(i)[2:] + '0'*(4 - len(bin(i)[2:])) for i in range(2**N)]
```
