...
def apply_two_qubit_gate(gate_matrix, pos, state, chi=1):
    """
    Apply a 2-qubit gate specified by the unitary matrix @gate_matrix to qubits at sites (@pos, @pos+1)
...
pos = 0
state2 = apply_two_qubit_gate(CNOT, pos, state1, chi=1)
...
def cphase_and_swap(phase):
    cphase = np.array(qu.controlled('z')) ** phase
    swap = np.array(qu.swap())
...
```{python}
#TODO: add np.conjugate to left/right_contract to allow gates/states with complex numbers [OK]
#(maybe it should be added even in other contractions... check!)
#Check if normalization works as expected [Appears so, but more testing is needed]
#Add truncation to fixed bond dimension chi. [TO DO] (And verify that the approximation is good)
#Make a lot of tests! [TO DO]
