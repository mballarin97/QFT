...
def cphase_swap(phi, dtype=complex, sparse=False):
...
psi0 = qu.tensor.MPS_computational_state('0' * n, tags = 'psi0') #Generate a (pure separable) state |00>

psi0.gate_(H, 0, tags='H', contract=False) 
psi0.gate_(cphase_swap(np.pi / 2), (0,1), tags='Cphase_swap')

psi0.gate_(H, 0, tags='H', contract=False) 
psi0.gate_(cphase_swap(np.pi / 4), (1,2), tags='Cphase_swap')
psi0.gate_(cphase_swap(np.pi / 2), (0,1), tags='Cphase_swap')

psi0.gate_(cphase_swap(np.pi / 8), (2,3), tags='Cphase_swap')
...
def qft_circuit(mps, n):
    ...
    mps.gate_(H, 0, tags='H', contract=False)
    for i in range(n-1):
        mps.gate_(cphase_swap(np.pi /2**(i+1)), (i, i+1), tags='Cphase_swap')
        
    return qft_circuit(mps, n-1)
```

```{python}
n = 4

psi0 = qu.tensor.MPS_computational_state('1000', tags = 'psi0') #Generate a (pure separable) state |00>
qft_circuit(psi0, n)
...
```{python}
#Objective: Apply CNOT to first two qubits, at positions (pos, pos+1)
#Assume pos=0 (i.e. first qubit is at left boundary)
...
state = to_full_MPS(state, d=2, N=4) #convert to MPS

#Define gate
pos = 0
CNOT = qu.controlled('not')
gate = np.array(CNOT.reshape(2,2,2,2))

#Rename qubits
# (left_tens) - (right_tens) -chi- (right_boundary) - (rightmost...)
#      |d             |d                 |              | | | |

left_tens = np.array(state[pos])
right_tens = np.array(state[pos+1])
right_boundary = state[pos+2]
rightmost = state[pos+3:]

#Dimensions
d = left_tens.shape[0] #dimension of a qubit
chi = right_tens.shape[-1] #dimension of link between right_tens and right_boundary
print(f'd: {d}, chi: {chi}')

#Contraction with gate
gate_contracted = ncon([gate, left_tens, right_tens], [[-1, -2, 1, 3], [1, 2], [2, 3, -3]])

#The network is now:
# (gate_contracted) -chi- (right_boundary) - (rightmost...)
#   |         |                 |               | | | | 
#We need to set gate_contracted as the center of orthogonality, so that when it is split into two MPS states with SVD,
#the approximation error is minimized.
#We proceed by following the direct orthogonalization method (https://www.tensors.net/p-tutorial-3 fig. 3.3(e))

#First we need the branch density matrix (fig.3.3(d)). For this we contract ALL tensors to the right of right_tens
rho = right_contract([right_boundary] + rightmost) #dim is chi x chi

#Then we diagonalize it
eigenval, eigenvec = LA.eigh(rho)
...
gate_contracted = ncon([gate_contracted,X],[[-1,-2,1],[1,-3]])
right_boundary = ncon([Xinv, right_boundary], [[-1,1], [1,-2,-3]])

#Now we can apply SVD to split gate_contracted in two tensors
U, S, Vh = LA.svd(gate_contracted.reshape(d, -1), full_matrices=False)
left_tens = U
right_tens = np.diag(S) @ Vh
right_tens = right_tens.reshape(-1, d, chi)
new_qubits = [left_tens, right_tens, right_boundary, *rightmost]

#|0000> -> |0000>
#|1000> -> |1100> 

to_dense(new_qubits).flatten()
```
