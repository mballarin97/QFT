...
```{python}
fig, ax = plt.subplots(1, 2, figsize=get_figsize(.95)) #the first parameter is the width fraction. .45 = half-page (spans one column), .95 = full-page (spans both columns) (accounting for a .05 margin)
#fig, ax = plt.subplots(1, 2, figsize=(9, 3))

#Comparison of "full" computations of QFT
ax[0].plot(N_quimb, times_quimb_full[:, 0], color='navy', label='quimb')
ax[0].fill_between(N_quimb, times_quimb_full[:,0]-times_quimb_full[:,1], 
                   times_quimb_full[:,0]+times_quimb_full[:,1],
                   color='cyan', alpha=0.5)

ax[0].plot(N_manual, times_manual_full[:, 0], 'r--', label='manual')
ax[0].fill_between(N_manual, times_manual_full[:,0]-times_manual_full[:,1], 
...
ax[1].plot(N_quimb, times_quimb_approx[:, 0], color='navy', label='quimb')
ax[1].fill_between(N_quimb, times_quimb_approx[:,0]-times_quimb_approx[:,1], 
                   times_quimb_approx[:,0]+times_quimb_approx[:,1],
                   color='cyan', alpha=0.5)

ax[1].plot(N_quimb, times_manual_chi[:, 0], 'r--', label='manual')
ax[1].fill_between(N_quimb, times_manual_chi[:,0]-times_manual_chi[:,1], 
                   times_manual_chi[:,0]+times_manual_chi[:,1],
                   color='pink', alpha=0.5)

#ax[1].set_ylabel('Time [s]')
ax[1].set_xlabel('Number of qubits')
ax[1].set_title('Time scaling for QFT circuit - $\chi=2$')
...
#palette = plt.cm.rainbow(np.linspace(0,1,4))

#Manual implementation
ax[0].plot(Ns, times_manual_chi[:, 0], 'r--', label='$\chi=2$', alpha=0.8)
...
for i in range(len(chis)):
    ax[0].plot(Ns, time_chis[i, :, 0], label='$\chi='+str(chis[i])+'$', 
               color=palette(i))#, alpha=0.2*(len(chis)-i))
    ax[0].fill_between(Ns, time_chis[i, :, 0]-time_chis[i, :, 1], 
                       time_chis[i, :, 0]+time_chis[i, :, 1],
                       color=palette(i), alpha=0.1*(len(chis)-i))

ax[0].set_ylabel('Time [s]')
ax[0].set_xlabel('Number of qubits')
ax[0].set_title('Time scaling for manual QFT circuit')
ax[0].legend(loc='best')
ax[0].set_yscale('log')

#Quimb implementation
ax[1].plot(Ns, times_quimb_approx[:, 0], 'r--', label='$\chi=2$', alpha=0.8)
...
ax[1].set_xlabel('Number of qubits')
ax[1].set_title('Time scaling for quimb QFT circuit')
ax[1].legend(loc='best')
ax[1].set_yscale('log')
...
times_quimb_auto = np.load("times_quimb_auto.npy")
times_manual_tot = np.load("times_manual_tot.npy")
times_cirq = np.load("times_cirq.npy")
times_qiskit = np.load("times_qiskit.npy")
...
```{python}
fig, ax = plt.subplots(1, 1, figsize=(6.704026567040266, 4.143316279913161)) #the first parameter is the width fraction. .45 = half-page (spans one column), .95 = full-page (spans both columns) (accounting for a .05 margin)
#fig, ax = plt.subplots()

#quimb implementation
plt.plot(N_tot, times_quimb_auto[:, 0], color='dodgerblue', label='quimb')
plt.fill_between(N_tot, times_quimb_auto[:,0]-times_quimb_auto[:,1], 
                   times_quimb_auto[:,0]+times_quimb_auto[:,1],
                   color='aquamarine', alpha=0.6)

#manual implementation
plt.plot(N_man_tot, times_manual_tot[:, 0], 'r--', label='manual, $\chi=2$')
plt.fill_between(N_man_tot, times_manual_tot[:,0]-times_manual_tot[:,1], 
                   times_manual_tot[:,0]+times_manual_tot[:,1],
                   color='pink', alpha=0.6)

#cirq
plt.plot(N_cirq, times_cirq[:, 0], color='orange', label='cirq')
plt.fill_between(N_cirq, times_cirq[:,0]-times_cirq[:,1], 
                   times_cirq[:,0]+times_cirq[:,1],
                   color='bisque', alpha=0.6)

#qiskit
plt.plot(N_qiskit, times_qiskit[:, 0], color='springgreen', label='qiskit')
plt.fill_between(N_qiskit, times_qiskit[:,0]-times_qiskit[:,1], 
                   times_qiskit[:,0]+times_qiskit[:,1],
                   color='palegreen', alpha=0.6)
...
fig.tight_layout()
plt.savefig("max_n_qubits.pdf") #Save as pdf in the "report_plots" folder
