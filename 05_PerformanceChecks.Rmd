...
ax[0].plot(x_fit, 10**(straight_line(np.log10(x_fit), *popt_q_approx)), '--r', 
           linewidth=0.8, label=f'$b=({np.round(b_q_approx, 1)}\\pm{np.round(err_b_approx, 1)})$') 
ax[0].plot(x_fit, 10**(straight_line(np.log10(x_fit), *popt_man)), '--g', 
           linewidth=0.8, label=f'$b=({np.round(b_man, 1)}\\pm{np.round(err_b_man, 1)})$') 
...
ax[1].plot(x_fit, 10**(straight_line(np.log10(x_fit), *popt_q)), '--r', 
           linewidth=0.8, label=f'$b=({np.round(b_q, 1)}\\pm{np.round(err_b_q, 1)})$') 


#ax[1].set_ylabel('Time [s]')
ax[1].set_xlabel('Number of qubits')
ax[1].set_title('Time scaling fot QFT - $\chi=2^{\\left \\lfloor n/2 \\right \\rfloor}$')
